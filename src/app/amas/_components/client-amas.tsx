...
export interface ClientAMAsProps {
  initialPosts: Post[];
...
function ClientAMAs({ 
  ...
  useEffect(() => {
    ...
    // Use the filterPosts function from client-utils
  ...
  const updateUrlParams = (newParams: { q?: string; topics?: string[]; page?: number }) => {
    ...
    const newUrl = `/amas${params.toString() ? `?${params.toString()}` : ''}`;
    console.log('New URL:', newUrl);
    
    // Use window.location for more reliable navigation with parameters
  ...
  // Function to handle direct navigation to AMA pages
  const navigateToAMA = (slug: string) => {
    try {
      // Navigate to the individual AMA post
        ...
        <div className="md:w-3/4">
          ...
          <div className="mb-6">
            <p className="text-gray-600">
              {isLoading ? (
                ...
                'No AMAs found. Try adjusting your filters.'
              ) : (
                `Showing ${filteredPosts.length} of ${totalPosts} AMAs`
              )}
            </p>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              <h3 className="text-xl font-semibold mb-2">No AMAs Found</h3>
              <p className="text-gray-600">
